...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  return <main className="min-h-screen bg-[#e9e8e7] text-[#2f3030] md:flex md:p-3">
    <aside className="hidden w-63.75 shrink-0 flex-col rounded-sm bg-white px-5 py-5 shadow-sm md:flex">
      <div className="flex items-center gap-2 text-[22px] font-extrabold tracking-[-1px]"><span className="grid h-10 w-10 place-items-center rounded-lg bg-[#303131] text-white">AE</span>AIEvaluator</div>
    ...
    <section className="flex min-h-screen min-w-0 flex-1 flex-col md:ml-3 md:min-h-[calc(100vh-24px)] md:rounded-sm md:bg-[#f5f4f3]">
      <header className="flex h-14 items-center justify-between border-b border-[#e9e7e5] bg-white px-4 sm:px-7 md:rounded-t-sm"><div className="flex items-center gap-3"><ChevronLeft className="h-5 w-5" /><span className="text-sm font-medium text-[#aaa7a5]">▣ &nbsp; Exams</span></div><div className="flex items-center gap-4"><Menu className="h-5 w-5 md:hidden" /></div></header><div className="min-h-0 flex-1">{children}</div>
    </section>
  </main>;
}
